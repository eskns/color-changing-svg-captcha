...
const getLineNoise = function (width, height, options) {
	...
	while (++i < options.noise) {
		...
		const start = `${random.int(1, 22)} ${random.int(1, height)}`;
		const end = `${random.int(width - 21, width)} ${random.int(1, height)}`;
		const mid1 = `${random.int((width / 2) - 21, (width / 2) + 22)} ${random.int(1, height)}`;
		const mid2 = `${random.int((width / 2) - 21, (width / 2) + 22)} ${random.int(1, height)}`;
		const color = hasColor ? random.color() : random.greyColor(min, max);
		//noiseLines.push(`<path d="M${start} C${mid1},${mid2},${end}" stroke="${color}" fill="none"/>`);
		noiseLines.push({ d: `M${start} C${mid1},${mid2},${end}`, stroke: `${color}`, fill: "none" });
	}
...
const getText = function (text, width, height, options) {
	...
	while (++i < len) {
		...
		out.push({fill: `${color}`, d: `${charPath}`});
	}
